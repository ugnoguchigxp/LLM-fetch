...
/// Narrow public wrapper for search-result guard checks (WF-08).
///
/// Inspects bounded plain text (provider + title + snippet + URL) as visible
/// text with `RequestedContextUse::AnswerWithCitation`. DOM segments and
/// internal rules stay private. Returns the guard decision plus warning
/// categories so callers can filter hits and count `blockedResultCount`.
/// Input over `max_characters` is truncated and treated as fail-closed
/// (`deny` when truncated, mirroring the truncation rule).
...
mod tests {
    ...
    #[test]
    fn plain_text_wrapper_allows_benign_and_fails_closed_on_truncation() {
